...
public class TicketSpecificaiton {
	...
	public static Predicate getPrediction(CriteriaQuery<?> query, Root<Ticket> root, CriteriaBuilder criteriaBuilder,
		...
		if (fligth_date != null) {
			Join<Flight, Ticket> flightAirline = root.join("airlineTicket");
			predicates.add(criteriaBuilder.and(criteriaBuilder.equal(flightAirline .get("flightDate"), fligth_date)));
		}
